...
shuanshu   = 0b0101010101010101    #单数按下状态
danshu     = 0b1010101010101010    #双数按下状态
...
adcConfig = 830    #830-25到830+25，805~855
...
def checkADC():
    ...
    if isShowADC:
        print(raw_value)
    if raw_value < adcConfig + rangeConfig and raw_value > adcConfig - rangeConfig: #
        return True #有福袋
    else:
        return False #没有福袋
...
def touchOnce(pins):
    dt_touch = randint(40,100)
    setAllPinStates(pins)  #点福袋
    time.sleep_ms(dt_touch)
    setAllPinStates(0xffff)  #点福袋
    time.sleep_ms(dt_touch)
    ...
    while True:
        ...
        if isRUN:
            ...
            if checkADC():               #程序运行时检测ADC值,界面出现时,ADC值小于300
                print('run:yes,','fdc:',fdcount,'nofdc:',nofdcount)
                if not isFDOK:
                    fdcount -= 1
                    if fdcount <= 0:
                        dt = randint(3,5)
                ...
                print('run:no,','fdc:',fdcount,'nofdc:',nofdcount)
                if isFDOK:
                    nofdcount -= 1
                    if nofdcount < 0:
                        isFDOK = False
