...
#获取父目录
def GetParentPath(strPath = None):
    if not strPath or strPath == '.':
        strPath = cur_file_dir()
...
def getAllExtFile(pth, fromatx=".erl", depth=-1):
    jsondir = pth
    jsonfilelist = []
    
    # 计算根目录的深度级别
    root_depth = pth.rstrip(os.sep).count(os.sep)
    ...
    for root, _dirs, files in os.walk(jsondir):
        ...
        for filex in files:
            ...
            # 检查文件扩展名
            if cmp(text, fromatx) == 0 or fromatx == ".*":
                jsonArr = []
                rootdir = pth
                dirx = root[len(rootdir):]
                pathName = dirx + os.sep + filex
